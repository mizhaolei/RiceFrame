...
//                添加行为记录
    ...
    public function repair($ids = null){
        if($ids) {
            if(is_array($ids)){
                ...
                if($list){
                    //                添加行为记录
                    action_log("databases_repair","",0,UID);
    ...
    public function del($time = 0){
        if($time){
            ...
            if(count(glob($path))){
                ...
                //                添加行为记录
                action_log("databases_del","",0,UID);
    ...
    public function export($ids = null, $id = null, $start = null){
        if(request()->isPost() && !empty($ids) && is_array($ids)){ //初始化
            ...
            //创建备份文件
            $Database = new \zhuopro\Database($file, $config);
            if(false !== $Database->create()){
                $tab = array('id' => 0, 'start' => 0);
                $this->success('初始化成功！', '', array('tables' => $ids, 'tab' => $tab));
            } else {
                $this->error('初始化失败，备份文件创建失败！');
            }
        } elseif (request()->isGet() && is_numeric($id) && is_numeric($start)) { //备份数据
            $ids = session('backup_tables');
            //备份指定表
            $Database = new \zhuopro\Database(session('backup_file'), session('backup_config'));
            $start  = $Database->backup($ids[$id], $start);
            if(false === $start){ //出错
                $this->error('备份出错！');
            } elseif (0 === $start) { //下一表
                if(isset($ids[++$id])){
                    $tab = array('id' => $id, 'start' => 0);
                    $this->success($ids[$id].'备份完成！', '', array('tab' => $tab));
                } else { //备份完成，清空缓存
                    ...
                    //                添加行为记录
                    action_log("databases_export","",0,UID);
                    $this->success('备份完成！');
                }
            } else {
    ...
    public function import($time = 0, $part = null, $start = null){
        if(is_numeric($time) && is_null($part) && is_null($start)){ //初始化
            ...
            $db = new \zhuopro\Database($list[$part], array(
            ...
            if(false === $start){
                ...
                if(isset($list[++$part])){
                    ...
                    //                添加行为记录
                    action_log("databases_import","",0,UID);
                    $this->success('还原完成！');
                }
            } else {
